...
function showToast(message, type = 'info', title = '', duration = 4000) {
    ...
    toast.innerHTML = `
        <div class="toast-header">
            <span>${escapeHTML(titles[type])}</span>
...
function handleFileSelect(file) {
    ...
    if (!validTypes.includes(file.type)) {
        showToast('Please select a valid image or PDF file', 'error', 'Invalid File Type');
        return;
    }
    ...
    showToast(`${escapeHTML(file.name)} loaded successfully`, 'success', 'File Loaded');
}
    ...
    reader.onload = (e) => {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                showToast('Document processed successfully', 'success', 'Processing Complete');
            } else {
                updateStatus(`Processing failed: ${escapeHTML(data.error || 'Unknown error')}`, 'error');
                showToast(escapeHTML(data.error || 'Unknown error'), 'error', 'Processing Failed');
            }
        })
        .catch(err => {
            document.getElementById('processingIndicator').style.display = 'none';
            ...
            showToast(`Failed to connect to OCR server: ${err.message}`, 'error', 'Connection Error');
        });
    };
...
function displayResults(data) {
    ...
    document.getElementById('docType').textContent = escapeHTML(data.document_type || 'Unknown');
    document.getElementById('classificationConf').textContent = ((data.classification_confidence || 0) * 100).toFixed(1);
    document.getElementById('ocrConf').textContent = ((data.ocr_confidence || 0) * 100).toFixed(1);
    document.getElementById('qualityScore').textContent = ((data.quality_score || 0) * 100).toFixed(1);
    document.getElementById('procTime').textContent = ((data.processing_time || 0) * 1000).toFixed(0);
    ...
    extractedText.textContent = escapeHTML(currentExtractedText || 'No text was extracted from the document.');
    extractedText.className = 'text-output' + (currentExtractedText.match(/[\u0D80-\u0DFF]/) ? ' sinhala' : '');
...
function speakText() {
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(escapeHTML(data.error || 'Unknown error'), 'error', 'TTS Failed');
        }
    })
    .catch(err => {
        updateStatus(`TTS error: ${err.message}`, 'error');
        showToast(`Text-to-speech connection failed: ${err.message}`, 'error', 'TTS Error');
    });
}
...
function clearResults() {
    ...
    document.getElementById('extractedText').textContent = 'Extracted text will appear here after processing...';
    ...
    fetch(`${BASE_URL}/api/ocr/stats`)
        ...
        .then(data => {
            if (data.success) {
                ...
                showToast(escapeHTML(data.error || 'Failed to load analytics'), 'error', 'Analytics Error');
            }
        })
        .catch(err => {
            showToast(`Failed to load analytics: ${err.message}`, 'error', 'Analytics Error');
        });
}
...
function updateAnalyticsDisplay(stats) {
    ...
    const successRate = totalDocs > 0 ? ((successful / totalDocs) * 100).toFixed(1) : 0;
    const identificationRate = totalDocs > 0 ? (((totalDocs - stats.errors || 0) / totalDocs) * 100).toFixed(1) : 0;
    const avgTime = (stats.avg_processing_time || 0) * 1000;

    document.getElementById('totalDocs').textContent = totalDocs;
    document.getElementById('successRate').textContent = successRate + '%';
    document.getElementById('identificationRate').textContent = identificationRate + '%';
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#34495e', '#2ecc71', '#3498db', '#9b59b6', '#e74c3c'],
                borderColor: '#ffffff',
                borderWidth: 2
            }]
        },
        ...
        data: {
            ...
            datasets: [{
                ...
                data: [
                    ...
                    stats.total_documents > 0 ? ((stats.errors || 0) / stats.total_documents * 100).toFixed(1) : 0,
                    stats.total_documents > 0 ? (((stats.total_documents - (stats.errors || 0)) / stats.total_documents) * 100).toFixed(1) : 0
                ],
        ...
        .catch(err => {
            document.getElementById('recentDocuments').innerHTML = `
                <div class="empty-state">
                    ...
                    <p>Failed to load document history: ${escapeHTML(err.message)}</p>
                </div>
            `;
        });
}
...
function displayHistory(results) {
    ...
    results.forEach(result => {
        ...
        const classificationConfidence = result.classification_confidence ? (result.classification_confidence * 100).toFixed(1) : 'N/A';
        const quality = result.image_quality ? (result.image_quality * 100).toFixed(1) : 'N/A';
        
        html += `
            <div class="document-card">
                <div class="document-header">
                    <div>
                        <div class="document-type">${escapeHTML(result.document_type || 'Unknown')}</div>
    ...
    fetch(`${BASE_URL}/api/ocr/daily_report?date=${date}`)
        ...
        .catch(err => {
            reportContent.innerHTML = `
                <div class="empty-state">
                    <h3>Error Loading Report</h3>
                    <p>Failed to load daily report: ${escapeHTML(err.message)}</p>
                </div>
            `;
        });
...
function displayDailyReport(data) {
    ...
    if (Object.keys(docTypes).length > 0) {
        docTypesHtml = '<div style="margin: 20px 0;"><h4>Document Type Distribution:</h4><div style="display: flex; flex-wrap: wrap; gap: 10px;">';
        for (const [type, count] of Object.entries(docTypes)) {
            docTypesHtml += `<span class="document-type">${escapeHTML(type)}: ${count}</span>`;
        }
    ...
    if (insights.length > 0) {
        insightsHtml = '<div class="chart-container"><h3 class="section-title">Insights</h3><ul class="insights-list">';
    ...
    reportContent.innerHTML = `
        <div class="analytics-grid">
            ...
            <div class="stat-card success-card">
                ...
                <div class="stat-value">${summary.ocr_success_rate ? summary.ocr_success_rate.toFixed(1) : 0}%</div>
                <div class="stat-label">Text Extraction</div>
            </div>
            <div class="stat-card identification-card">
                <h4>Identification Rate</h4>
                <div class="stat-value">${summary.identification_success_rate ? summary.identification_success_rate.toFixed(1) : 0}%</div>
                <div class="stat-label">Type Classification</div>
            </div>
            <div class="stat-card time-card">
                <h4>Avg Time</h4>
                <div class="stat-value">${summary.avg_processing_time ? (summary.avg_processing_time * 1000).toFixed(0) : 0}ms</div>
        ...
        <div class="chart-container">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                <div>
                    ...
                    <div class="stat-value" style="font-size: 2.5em;">${summary.avg_quality ? (summary.avg_quality * 100).toFixed(1) : 0}%</div>
                </div>
                <div>
                    <h4>Average Confidence</h4>
                    <div class="stat-value" style="font-size: 2.5em;">${summary.avg_confidence ? (summary.avg_confidence * 100).toFixed(1) : 0}%</div>
                </div>
            </div>
        </div>
...
function loadProcessingLogs(date) {
    ...
    logsDiv.innerHTML = '<div style="text-align: center; padding: 20px;"><span class="loading-spinner"></span>Loading processing logs...</div>';
    ...
    fetch(`${BASE_URL}/api/ocr/processing_logs?date=${date}`)
        ...
        .catch(err => {
            logsDiv.innerHTML = `
                <div class="empty-state">
                    <h3>Error Loading Logs</h3>
                    <p>Failed to load processing logs: ${escapeHTML(err.message)}</p>
                </div>
            `;
        });
}
...
function displayProcessingLogs(data) {
    ...
    logs.forEach(log => {
        ...
        const statusText = escapeHTML(log.status || (log.success ? 'Success' : 'Failed'));
    ...
    fetch(`${BASE_URL}/api/ocr/historical_data?days=7`)
        ...
        .catch(err => {
            historicalDiv.innerHTML = `
                <div class="empty-state">
                    <h3>Error Loading Historical Data</h3>
                    <p>Failed to load historical data: ${escapeHTML(err.message)}</p>
                </div>
            `;
        });
}
...
function displayHistoricalData(data) {
    ...
    days.forEach(day => {
        historicalHtml += `
            <tr style="background: white; border-bottom: 1px solid #f0f0f0;">
                <td style="padding: 12px 15px; font-weight: 600;">${escapeHTML(day.date)}</td>
                <td style="padding: 12px 15px; text-align: center; font-weight: 700;">${day.total_documents || 0}</td>
                <td style="padding: 12px 15px; text-align: center; color: #4caf50; font-weight: 700;">${day.ocr_success_rate ? (day.ocr_success_rate * 100).toFixed(1) + '%' : 'N/A'}</td>
                <td style="padding: 12px 15px; text-align: center; font-weight: 600;">${day.avg_quality ? (day.avg_quality * 100).toFixed(1) + '%' : 'N/A'}</td>
                <td style="padding: 12px 15px; text-align: center; font-weight: 600;">${day.avg_processing_time ? (day.avg_processing_time * 1000).toFixed(0) + 'ms' : 'N/A'}</td>
            </tr>
        `;
    });
...
function generateTestData() {
    ...
    .then(data => {
        if (data.success) {
            showToast('Test data generated successfully!', 'success', 'Data Generated');
            loadDailyReport();
            if (document.getElementById('analytics-tab').classList.contains('active')) {
                loadAnalytics();
            }
        } else {
            showToast(`Failed to generate test data: ${escapeHTML(data.error || 'Unknown error')}`, 'error', 'Error');
        }
    })
    .catch(err => {
        showToast(`Failed to generate test data: ${err.message}`, 'error', 'Error');
    });
}
...
startBtn.textContent = 'Start Camera';
showToast(`Failed to start camera: ${err.message}`, 'error', 'Camera Error');
});
}
...
.catch(err => {
showToast(`Error stopping camera: ${err.message}`, 'warning', 'Camera Warning');
});
}
...
.then(data => {
    if (data.image) {
        ...
        if (img.src) {
            ...
            canvas.toBlob((blob) => {
                currentDocumentData = new File([blob], 'camera-capture.jpg', { type: 'image/jpeg' });
        ...
        .then(data => {
            ...
            if (statusEl) {
                if (data.model_loaded && data.ocr_ready) {
                    ...
                    showToast('OCR system initialized successfully', 'success', 'System Ready');
                } else {
                    statusEl.innerHTML = 'System partially ready - Check server status';
                    statusEl.className = 'status warning';
                    showToast('OCR system partially initialized', 'warning', 'Partial Init');
                }
            }
        })
        .catch(err => {
            ...
            showToast(`Cannot connect to OCR server: ${err.message}`, 'error', 'Connection Failed');
        });
